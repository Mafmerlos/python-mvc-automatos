...
from automato_inicia_0_termina_1 import validar as validar_inicia_0_termina_1
from automato_termina_01 import validar as validar_termina_01
from automato_inicia_11 import validar as validar_inicia_11
...
class Controller:
    ...
    def __init__(self, view):
        ...
        self.validation_map = {
            ...
            "Inicia com 0 termina com 1": validar_inicia_0_termina_1,
            "Termina com 01": validar_termina_01,
            "Inicia com 11": validar_inicia_11
        }
        ...
        self.automaton_file_map = {
            ...
            "Inicia com 0 termina com 1": "automato_inicia_0_termina_1.py",
            "Termina com 01": "automato_termina_01.py",
            "Inicia com 11": "automato_inicia_11.py"
    ...
    def importar_arquivos_para_bd(self):
        ...
        try:
            ...
            self.view.exibir_cabecalho_arquivo(
                "Importando arquivos para o banco de dados...")

            total_arquivos, total_palavras = self.db_manager.importar_arquivos_txt(
                filepaths)
            ...
            if total_palavras > 0:
                ...
                self.view.exibir_erro(
                    "Os arquivos selecionados estavam vazios ou não puderam ser lidos.")
    ...
    def processar_palavras_do_bd(self):
        ...
        self.view.limpar_resultados()
        self.view.exibir_cabecalho_arquivo(
            f"Processando com o Autômato: '{selected_automaton_name}'")
        ...
        try:
            ...
            if not dados_do_banco:
                self.view.exibir_erro(
                    "Nenhuma palavra encontrada no banco. Importe os arquivos primeiro.")
                return

            for nome_arquivo, palavras in dados_do_banco.items():
                self.view.exibir_cabecalho_arquivo(
                    f"Resultados de '{nome_arquivo}'")
                ...
                for palavra in palavras:
                    ...
                    dados_para_salvar = (
                        palavra, nome_arquivo, selected_automaton_name, resultado_str)
                    resultados_para_salvar_no_bd.append(dados_para_salvar)

            if resultados_para_salvar_no_bd:
                self.db_manager.salvar_validacao_em_lote(
                    resultados_para_salvar_no_bd)
                self.view.exibir_cabecalho_arquivo(
    ...
    def visualizar_historico(self):
        try:
            self.view.limpar_resultados()
            self.view.exibir_cabecalho_arquivo(
                "Consultando Histórico Completo do BD...")
            ...
            for registro in historico:
                ...
                self.view.exibir_linha_historico(
                    data_formatada, automato, palavra, res)
    ...
    def solicitar_reset_banco(self):
        if self.view.confirmar_reset():
            try:
                ...
                self.view.exibir_erro(
                    "Todo o histórico e palavras importadas foram apagados.")
            except Exception as e:
    ...
    def mostrar_codigo_automato(self):
        try:
            ...
            if not filepath:
                self.view.exibir_erro(
                    f"Não foi possível encontrar o arquivo de código para '{selected_automaton_name}'.")
                return
            ...
            if not os.path.exists(filepath):
                self.view.exibir_erro(
                    f"Erro Crítico: Arquivo não encontrado no diretório do projeto: {filepath}")
                return

            self.view.exibir_cabecalho_arquivo(
                f"Exibindo Código-Fonte de: {filepath}")
